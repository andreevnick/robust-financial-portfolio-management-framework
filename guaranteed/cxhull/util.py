...
def get_max_coordinates(x, f, z, method='interior-point', tol=1e-8, debug_mode=False, ignore_warnings=False):
    """ Solves the problem
        ...
        p_1 + ... + p_m = 1,
        p_1, ... p_m >= 0
    w.r.t. m-dimensional vector p and returns the optimal p. Equivalently, finds the baricentric coordinates
    ...
    if res.status > 0:
        ...
        raise Exception('unable to find the convex representation of z')
...
def in_triangle_2D(z, points, tol=1e-12):
    ...
    if is_coplanar_2D(points, tol=tol):
        ...
        for ind in [[1,2], [0,2], [0,1]]:
            ...
            alpha = ((z-points[ind[1]]) @ (points[ind[0]]-points[ind[1]])) / ((points[ind[0]]-points[ind[1]]) @ (points[ind[0]]-points[ind[1]]))
        
            if (alpha >= 0) and (alpha <= 1) and (np.max(np.abs(z - points[ind[1]] - alpha*(points[ind[0]]-points[ind[1]]))) <= tol):
