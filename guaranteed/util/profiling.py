...
class Timer:
    ...
    def __init__(self, header=None, flush=False, silent=False):
        ...
        self.header = coalesce(header, 'Elapsed time')
        self.flush = flush
        self.silent = silent
    ...
    def fancystr(hms):
        ...
        if hms[0] > 0: res += '{0} hr '.format(hms[0])
        if (res != '') or (hms[1] > 0): res += '{0} min '.format(hms[1])
        if ((res != '') and (hms[2] > 0)) or (res == ''): res += '{0:.4f} sec'.format(hms[2])
...
class ProfilerData:
    ...
    def __init__(self, header=None):
        ...
        self.header = coalesce(header, 'Total time')
